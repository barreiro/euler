...
public final class Combinatorics {
    ...
    private static class Permutator implements Iterator<long[]> {
        ...
        private Permutator(Set<Long> set) {
            ...
            int index = 0;
            for (Long l : set) this.set[index++] = l.intValue();
        }
        ...
        public long[] next() {
            ...
            if (next == 0 || next == permutation.length - 1 || permutation[next + (forward[next] ? 1 : -1)] > max) fixed[next] = true;
    ...
    private static class Partitioner implements Iterator<long[]> {
        ...
        public long[] next() {
            ...
            for (int i = pivot.length - 1; i > 1; i--) {
                if (pivot[i] - 1 != pivot[i - 1]) {
                    pivot[i - 1]++;
                    if (i != pivot.length - 1) pivot[i] = pivot[i - 1] + 1;
                    break;
                }
            }
            return result;
        ...
        public boolean hasNext() {
            for (int i = pivot.length - 1; i > 1 ; i--) {
                if (pivot[i] - 1 != pivot[i - 1]) return true;
            }
            return false;
        }
