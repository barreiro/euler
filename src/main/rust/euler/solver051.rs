...
impl Solver for Solver051 {
    fn solve(&self) -> i64 {
        let predicate = |&p: &u64| {
            ...
            (0..=9 - u8::try_from(self.n).unwrap()).find(|&i| digit.iter().skip(1).filter(|&&d| d == i).count() >= required_repetitions).map_or(false, |repeated| {
                ...
                digit.first().unwrap() != &repeated && (0..=9).filter(|&substitute| {
