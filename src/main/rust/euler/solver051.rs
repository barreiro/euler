...
use crate::algorithm::digits::{Digit, Digits, from_raw_digits, skip_digits};
use crate::algorithm::filter::{is_prime, less_than_u64};
...
impl Solver for Solver051 {
    fn problem_name(&self) -> &str { "Prime digit replacements" }
    ...
    fn solve(&self) -> i64 {
        let predicate = |&p: &u64| {
            ...
            // find a digit that is repeated the required amount of times and replace its occurrences
            (0..=9 - self.n).find(|&i| digit.iter().skip(1).filter(|&&d| d == i).count() >= required_repetitions as usize).map_or(false, |repeated| {
                ...
                digit.first().expect("Digit should not be empty") != &repeated && (0..=9).filter(|&substitute| {
                    ...
                    // count the elements *not* in the family, to allow an early return
