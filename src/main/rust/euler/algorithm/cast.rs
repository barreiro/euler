...
impl Cast for char {
    ...
    fn as_u64(&self) -> u64 {
        u64::from(char_as_u32(*self))
    }
    fn as_usize(&self) -> usize {
        usize::try_from(char_as_u32(*self)).unwrap()
    }
}
...
impl Cast for Option<u64> {
    ...
    fn as_usize(&self) -> usize {
        usize::try_from(self.unwrap_or_default()).unwrap()
    }
}
...
impl Cast for Option<Digits> {
    fn as_i64(&self) -> i64 {
        i64::try_from(self.as_ref().map_or(0, Digits::value)).unwrap()
    }
    fn as_u64(&self) -> u64 {
        self.as_ref().map_or(0, Digits::value)
    }
    fn as_usize(&self) -> usize {
        usize::try_from(self.as_ref().map_or(0, Digits::value)).unwrap()
    }
}
...
#[must_use]
pub fn char_as_i64(c: char) -> i64 {
    i64::from(char_as_u32(c))
}
...
fn char_as_u32(c: char) -> u32 {
    if c >= '0' && c <= '9' {
       ...
       (c >= 'A').then_some(1 + c as u32 - 'A' as u32).or_else(|| c.to_digit(u32::from(DEFAULT_RADIX))).unwrap()
    }
}
