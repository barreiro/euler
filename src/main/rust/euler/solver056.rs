...
impl Solver for Solver056 {
    fn solve(&self) -> i64 {
        ...
        (floor..ceil).map(|a| vectorized_power(a.as_u64()).skip(floor - 1).take(ceil - floor).map(|power| power.into_iter().map(digits_sum).sum::<u64>().as_i64()).max().unwrap()).max().unwrap()
    }
}
