...
use euler::algorithm::long::{from_digits_array, is_even};
use euler::algorithm::prime::is_prime;
use euler::Solver;
...
impl Solver for Solver041 {
    fn solve(&self) -> isize {
        ...
        let predicate = |d: &[_]| if *d.last().unwrap() != d.len() as _ || is_even(*d.first().unwrap()) { None } else { Some(from_digits_array(d)).filter(is_prime) };
