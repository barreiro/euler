...
func solver011(N int) int {
	max, intGrid := math.MinInt64, gridStrToInt(input011)

	for n := 0; n < len(intGrid)-N; n++ {
...
func gridStrToInt(grid []string) [][]int {
	gridInt := make([][]int, 0)
	for _, g := range grid {
		parsed := make([]int, 0)
